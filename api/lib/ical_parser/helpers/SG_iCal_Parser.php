...
class SG_iCal_Parser {
	...
	public static function Parse( $url, SG_iCal $ical ) {
		...
		// Fetch returns false if file does not exist
		if($content !== false){
			$content = self::UnfoldLines($content);
			self::_Parse( $content, $ical );
			$ical->setParseStatus(true);
		} else {
			$ical->setParseStatus(false);
		}
	}
	...
	protected static function Fetch( $resource ) {
		...
		if( is_file( $resource ) ) {
			...
			$content = curl_exec($c);
			/**
			 * @simons 11JUL2015: Check if file exists!
			 *         Return false if not...
			 *         CURLINFO_HTTP_CODE >= 400 -> not found, CURLINFO_HTTP_CODE = 200, found.
			 */
